...
export class AuthService {
  ...
  async register(
    body: RegisterDTO, response: Response
  ): Promise<any> {
    ...
    response.cookie('api-auth', { accessToken, refreshToken }, {
      secure: true,
      httpOnly: true,
      expires: new Date(
        Number(new Date()) + this.config.get().cookieSecretExpiresDurationInMinutes * 60 * 1000,
      ),
      signed: true,
    });
  ...
  async login(
    ...
    const isMatched = await this.helper.comparePassword(password, userData.password);
  ...
  async logout(
    ...
    // set cache data
    await this.cacheManager.set(email, JSON.stringify(cacheData), this.config.get().cacheExpiresDurationInMinutes * 60,
    );
